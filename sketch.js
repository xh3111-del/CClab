...
class Bubble {
  ...
  display() {
    ...
    let a = map(this.age, 0, this.lifespan, 100, 10); // alpha in HSB 0..100
    ...
    colorMode(RGB);
    noStroke();
    fill(255, 255, 255, a * 2.55 * 0.9); // 转成 0-255 alpha
    ellipse(-this.size * 0.12, -this.size * 0.12, this.size * 0.22);

    fill(200, 200, 255, a * 2.55 * 0.5);
...
function dandelion() {
  ...
  for (let i = 0; i < 150; i++) {
    a = width/2 + random(-30, 30);
     b=height/2 + random(-30, 30);
    seeds.push(new Seed(a,b));
  }
...
function drawSideDecorations() {
  ...
  let N = 9;
  for (let i = 0; i < N; i++) {
    let x = map(i, 0, N - 1, -W / 2 + 40, W / 2 - 40);
